...
print()
  ...
  ## Resultater  Typefeil er ved 4mmol laktat er `r cv4`% og ved 2mmol laktat er den `r cv2`%
```

```{r}
<<<<<<< HEAD
# Create a data frame of known distances and molecular weights
ladder <- data.frame(dist = c(), 
                     mw = c(1000, 900, 800, 
                            700, 600, 500,
                            400, 300, 250, 
                            200, 150, 100, 50))
...
```
=======
>>>>>>> e971a3209c20d63f07bead899179bf3cae621f11
...
ladder <- data.frame(dist = c(29, 43.5, 60.5, #x-verdier skal inn her
                              80.5, 106.5,  141.5,  
                              181.5,    243.5,  281.5,
...
# Create a new data frame of unknowns
unknown <- data.frame(dist = c(470.5,   391.5,  528.5,
                               421.5,   507.5)) #overskuddsdata skal inn her. Like mange 

ladder %>%
  ggplot(aes(dist,log(mw)) + geom_point() + geom_smooth(method = "lm")
         

mod <- lm (log(mw) tilde dist, data = ladder)
