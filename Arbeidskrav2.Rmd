...
#filtr can be used to filter observations based on their value. Operations for comparisons can be used to subest the data set. ! x (NOT x) x & y (x AND y)X strek(??) Y (X eller Y). Test if a value is missing(NA), is a number or character. is.na (), is.numeric(),is.character() 

library(tidyverse)
library(exscidata); data("cyclingstudy")
cyclingstudy %>% 
...
#group.by functinos groups a data table (data frame or tibble). Dataseted deles opp. It can be used with mutate or summarise
...
#hvis nullhypotesen stemte, skulle resultatet 
